    ...
    DB: ReDatabase,
{
    ...
    pub(super) fn adjust_base_fees(&self, gas_accumulator: &GasAccumulator, block_number: u64) {
        ...
        // `--network` built-in). A node that booted always has one installed; only
        // in-process engines that skip the gate (test utilities) reach the `None`
        // arm, where an all-`Never` schedule is correct. If a node ever reaches it,
        // `debug_assert!` surfaces that loudly in tests.
