...
//! `epochs <FROM> <TO>` and `epoch-check`.
...
#[test]
fn epoch_check_finds_broken_link_gap_and_uncertified() {
    let fx = Fixture::new();
    ...
    assert!(!n.ok);
    assert_eq!(report.verdict.to_string(), "BROKEN nodes=1 checked=5 gaps=1 uncertified=1 first=3");
}

#[test]
fn epoch_check_range_beyond_the_latest_record() {
    let fx = Fixture::new();
    let a = SeededNode::new(|db| drop(seed_healthy(&fx, db)));
...
#[test]
fn epoch_check_broken_parent_hash() {
    let fx = Fixture::new();
    let a = SeededNode::new(|db| {
    ...
    assert_eq!(report.verdict.to_string(), "BROKEN nodes=1 checked=2 broken=1 first=2");
}

#[test]
fn epoch_check_divergent_nodes() {
    let fx = Fixture::new();
    let a = SeededNode::new(|db| drop(seed_healthy(&fx, db)));
    let b = SeededNode::new(|db| {
        let (records, _) = seed_healthy(&fx, db);
...
/// The digest index is part of the chain: a record whose digest maps elsewhere is reported.
#[test]
fn epoch_check_reports_index_mismatches() {
    let fx = Fixture::new();
    let a = SeededNode::new(|db| {
        let (records, _) = seed_healthy(&fx, db);
        db.with_write_txn(|txn| {
            rayls_infrastructure_types::DbTxMut::insert::<
                rayls_infrastructure_storage::tables::EpochRecordsIndex,
            >(txn, &records[1].digest(), &5)
        })
        .unwrap();
    });
    let report = epoch_check(&[(a.open("a"))], None, None, false).unwrap();
...
/// `-v` lists every record of the range: the focused view around one epoch.
#[test]
fn epoch_check_verbose_lists_records() {
    ...
    assert_eq!(records[1].committee_handoff_ok, Some(true));
    assert_eq!(records[1].parent_hash, records[0].digest);
