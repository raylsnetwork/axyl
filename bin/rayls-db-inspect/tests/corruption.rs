...
/// New nodes will use 16 KiB pages. MDBX guesses meta-page offsets from the 4 KiB system page,
/// so once the first page is destroyed it cannot find pages 1 and 2 on its own; the tool retries
/// with page-size hints and opens from a surviving meta page. All three destroyed is the end.
#[test]
fn a_16k_page_database_with_its_first_page_destroyed_still_opens() {
    use rayls_infrastructure_config::RaylsDirs as _;
    use rayls_infrastructure_storage::{mdbx::MdbxConfig, open_db_with_consensus_config};
    let fx = Fixture::new();
    let dirs = rayls_testing_test_utils::RaylsTempDirs::default();
    {
        let db = open_db_with_consensus_config(
            dirs.consensus_db_path(),
            &MdbxConfig { page_size: Some(16384), ..Default::default() },
        );
        seed_healthy(&fx, &db);
        db.sync_persist().unwrap();
    }
    let source = dirs.consensus_db_path();
    assert_eq!(page_size(&source.join("mdbx.dat")), 16384);
    let root = tempfile::tempdir().unwrap();
    for (zeroed_pages, must_open) in [(1u64, true), (2, true), (3, false)] {
        let copy = root.path().join(format!("p{zeroed_pages}"));
        sparse_copy(&source, &copy);
        ...
        match (must_open, opened) {
            (true, Ok(db)) => {
                ...
                // last three commits, so destroying the newer ones shows an older state (here
                // possibly one before any header was written), never a wrong one
            ...
            (false, Ok(_)) => panic!("all meta pages destroyed yet it opened"),
            (false, Err(_)) => {}
        }
    }
}
