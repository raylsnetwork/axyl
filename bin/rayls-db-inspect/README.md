...
|---|---|---|
| `epoch <EPOCH>` | record present, digest index consistent, certificate present; `epoch_hash` matches the record; signer count vs. super-quorum; BLS aggregate verifies; `parent_hash` links to record N-1; committee hand-off matches; boundary header resolves (and in which tier); leftover transition checkpoint; a pending record (the record the node built at the close, certificate not on disk yet) with its digest, parent, boundary and committee sizes, flagged `STALE` when a certified record exists beside it | `nodes variants certified genesis record_only pending missing not_reached stale_pending` |
| `epochs <FROM_EPOCH> <TO_EPOCH>` / `--all` | per epoch `RC` record+cert, `R-` record only, `P-` pending (closed, no certificate yet), `--` missing, `..` not reached yet, `??` table absent (presence only, no BLS check); row status `ok` / `partial` / `missing` / `not-reached` / `divergent` | `epochs ok partial missing divergent not_reached first` |
| `epoch-check [--from EPOCH] [--to EPOCH]` | gaps, broken `parent_hash` links, uncertified epochs, pending epochs (closed, awaiting the certificate; counted as uncertified, and the next record is expected to chain to the pending one), stale pending rows beside certified records, invalid certificates, committee hand-off mismatches, digest-index mismatches, nodes with no records although epochs have closed; `-v` lists every record in range (digest, parent, certificate state, index, link, hand-off, committee size); the default range spans certified and pending records, a range past the latest is clamped and noted; issue counts are summed over nodes, `checked` is the fullest node's count | `nodes checked divergent gaps broken uncertified pending invalid handoff index no_records stale_pending first` |
...
- A node saves the record it builds at an epoch's close to `pending_epoch_record` before the
  certificate exists, one row per closed-but-uncertified epoch, and the certificate's write
  removes the row in the same transaction. A row therefore means "closed here, not certified
  yet": `epoch` reports it as `pending` (with the record's fields), `epochs` shows `P-`,
  `epoch-check` lists the epoch under `pending` instead of as a gap and expects the next record
  to chain to it, and `summary` lists every pending epoch. A row that survives beside a
  certified record is a leftover the node's startup sweep should have removed; it is flagged
  `STALE`. A pending record is never trusted as a certified one: it counts as uncertified in
  every verdict.
